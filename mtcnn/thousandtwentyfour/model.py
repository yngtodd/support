...
class MTCNN(nn.Module):
    def __init__(self, wv_matrix, kernel1=3, kernel2=4, kernel3=5, num_filters1=100,
                 ...
                 max_sent_len=1500, word_dim=49, vocab_size=2881, subsite_size=8,
    ...
    def forward(self, x):
#        print('Input data is of type {}'.format(type(x.data)))
        x = self.embedding(x).view(-1, 1, self.word_dim * self.max_sent_len)
#        print('embedding is {}, {}'.format(type(x.data), x.size()))
...
#        print('fc1 out: {}, {}'.format(type(out_subsite.data), out_subsite.size()))
        out_laterality = self.fc2(x)
#        print('fc laterality out: {}, {}'.format(type(out_laterality.data), out_laterality.size()))
    ...
    def count_parameters(self):
       ...
       for name, p in self.named_parameters():
           if p.requires_grad:
              ...
              total_size += getsizeof(p.data)
              print('name=' + name + 'params=' + str(p.numel()) + ' size=' + str(getsizeof(p.data)))
       print('total_params = ' + str(total_params) + ' total_size=' + str(total_size)) 
       # print(' cuda_mem=' + str(torch.cuda.memory_allocated()))
